...
const Notifications = () => {
    ...
    const fetchNotifications = ({ page, deletedDocCount = 0 }) => {
        axios.post(import.meta.env.VITE_SERVER_DOMAIN + "/notifications", { filter, page, deletedDocCount }, {
            headers: {
            ...
            <div className="my-8 flex gap-6">
                {
                    filters.map((filterName, i) => {
                        return <button key={i} className={"py-2 px-4  " + (filter == filterName ? "btn-dark" : "btn-light")} onClick={handleFilter} >{filterName}</button>
                    })
                }
            </div>
            {
                    ...
                    <>
                        {
                            notifications.results.length ?
                                ...
                                }) : <NoDataMessgae message="No Notifications Found" />
                        }
